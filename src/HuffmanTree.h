...
class HuffmanTree {
public:
    static std::map<unsigned char, int> buildFrequency(const std::string& text); // Подсчет частот символов
    static Node* buildTree(const std::map<unsigned char, int>&freq); // Построение дерева
